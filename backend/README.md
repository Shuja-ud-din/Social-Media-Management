...
## Not Compatible with Node.js v19 and older
...
```bash
git clone --depth 1 https://github.com/sllark/instagram-youtube-backend.git
cd instagram-youtube-backend
```
...
1. **Node.js**: Version specified in `.nvmrc`. We recommend using `nvm` (Node Version Manager) for managing Node.js versions..

2. **Yarn**: We strongly recommend using `yarn` as the package manager for this application..

3. **Run pre installation script**: You must run this script before installing the dependencies using `yarn` only. Don't use any other node package manager instead of `yarn` fot this application.. 

    ```bash
    bin/preinstall
    # use bash shell like git bash to run this script
    ```
